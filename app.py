"""
app.py
Dissertation dashboard — Pinus taeda Yield Prediction
-----------------------------------------------------
Navigation  : sidebar radio buttons (Home + 3 project pages)
Layout      : scrollytelling — markdown narrative → chart → narrative → chart
...
# ---------------------------------------------------------------------------
# Cached plot builders — prevent re-generating heavy figures on every render
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
st.set_page_config(
    page_title="Pinus taeda · Yield Prediction Dashboard",
    page_icon="🌲",
    layout="wide",
...
| **UndPrp** | Proportion of returns in the understory compared to in the canopy (0–1) |
| **Z** | Tree top height |
...
""", unsafe_allow_html=True)
    ...
    st.caption(
        ...
        "Shaded bands show the 25th–75th percentile spread. "
...
*   **The "Free-to-Grow" Phase (Ages 9–11):** In the years immediately following the drone flight, yield predictions are dominated by Individual Tree Crown (ITC) metrics—such as top height (`Z`), crown volume (`vol5`), and surface area (`sfa2`). At this stage, a tree's future yield is primarily dictated by its own established size and photosynthetic capacity.
*   **The Onset of Canopy Closure (Ages 12–15):** As the prediction window extends, the importance of individual size plateaus or declines, while the height-based Competition Index (`CI_Z`) surges dramatically. By age 13, `CI_Z` emerges as the undisputed dominant predictor in both models.
...
However, all seven models exhibit a persistent pattern of heteroscedasticity driven by **signal saturation**. While the algorithms perform reasonably well at predicting low to moderate volumes (points clustering near the 1:1 line), they systematically "flatline" and underpredict yield in high-volume plots (field volumes > 125 m³/ha). This visualizes a fundamental limitation of optical and C-band SAR sensors: once a dense forest canopy closes, the satellite signal loses sensitivity to further biomass accumulation beneath the canopy, artificially capping the models' predictive range.
...
*   **The Operational Sweet Spot (618–1236 TPH):** All models achieve their highest accuracy (lowest RMSE) in medium-density stands. This represents the optimal balance: the canopy is sufficiently closed to provide a strong, coherent remote sensing signal, but not so dense that it overwhelms the sensors.
...
*   **Random Forest (RF) Prioritizes Optical Reflectance:** The RF model relies overwhelmingly on Sentinel-2's Short-Wave Infrared band (`B11`). As a non-temporal model, RF favors spectral bands that provide strong, instantaneous correlations with canopy water content and canopy closure—proxies for total biomass.
